...
| `stock_surplus_min_stock` | 20 | ЭВРИСТИКА ДЛЯ ДЕМО: склад без продаж (нет строки `days_of_stock`) с остатком выше порога становится донором «по остатку» (`reason=stock_surplus`); финальное решение — при реальном клиенте, см. `docs/roadmap.md` |
