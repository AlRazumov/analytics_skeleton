...
Промежуточные `stage*`-ветки (все — предки `master`) удалены на GitHub и
локально 2026-09-24. Кроме `master` осталась только `backup/stage6-contracts`
— отдельная резервная точка, в `master` не входит.
