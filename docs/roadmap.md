...
Репозиторий подключён к GitHub (`AlRazumov/analytics_skeleton`), все
локальные ветки запушены. `master` на GitHub соответствует состоянию до
этапа 07 — там ещё нет ни одного из этапов 07–17. Вся накопленная работа
(этапы 07–14, этап 17, фикс `resolvePeriod`/`ProvidesHistoryBounds`,
демо-эвристики `lost_sales`/`stock_no_demand`) лежит в ветке
`stage-14-review-fixes` — она линейный потомок `master` (65 коммитов
вперёд, 0 расхождений), сливается чисто.

Открыт один Pull Request: `stage-14-review-fixes` → `master`. Остальные
`stage-*`-ветки на GitHub — промежуточные точки той же линии истории (все
являются предками `stage-14-review-fixes`, кроме `backup/stage6-contracts`
— отдельная резервная точка); отдельные PR для них создавать не нужно,
их содержимое уже входит в PR `stage-14-review-fixes`. GitHub может
предлагать для них кнопку «Compare & pull request» — это не требует
никакого действия.
