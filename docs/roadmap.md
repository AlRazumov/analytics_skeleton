...
## Статус проекта (обновлено 2026-09-25)
...
- отложенное из code review 2026-09-24: `.env.example` с
  `DB_CONNECTION=sqlite` при Postgres-only коде (§4.1); на обзоре серия
  «год назад» — сплошной ноль, когда истории меньше двух лет (§3.1, как
  `yearAgoAvailable` на топ-товарах); разнобой форматирования чисел в
  `table`/`kpi-card` и CSV (§3.2);
...
## Git/GitHub (обновлено 2026-09-25)
