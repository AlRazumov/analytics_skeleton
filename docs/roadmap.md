...
| 08 | Bitrix24Adapter | — | not planned (отложен на неопределённый срок, ждёт клиента) | 06 |
| 09 | OneCAdapter | — | not planned (отложен на неопределённый срок; заказчик тянет время, может не состояться) | 06 |

## TODO before real adapters

Пункты, которые нужно пересмотреть перед стартом этапов 08/09 (реальные,
...
MockAdapter, часть 1) выполнен 2026-09-19; этап 07 (метрики остатков) — тоже. Этапы 08
(`Bitrix24Adapter`) и 09 (`OneCAdapter`) — not planned, ждут появления
...
клиента/доступа к Bitrix24/1С; следующий реальный шаг — начать этап 08
или 09, когда появится клиент.
