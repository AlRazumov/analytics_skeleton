...
| 12 | Bitrix24Adapter | — | not planned (отложен на неопределённый срок, ждёт клиента) | 06 |
| 13 | OneCAdapter | — | not planned (отложен на неопределённый срок; заказчик тянет время, может не состояться) | 06 |

## TODO before real adapters

Пункты, которые нужно пересмотреть перед стартом этапов 12/13 (реальные,
...
- ⚠️ open (2026-09-19, найдено при анализе HEAD `3ddd752`):
...
- ⚠️ open (2026-09-19, найдено при анализе HEAD `3ddd752`): «анти-топ» на
...
2. ⏳ `MockAdapter::fetchDeals()` и окно истории.
3. ⏳ Анти-топ: честный заголовок и пояснение про «Неликвиды».
4. ⏳ Запись в Known issues про потерянные продажи (docs).

## Статус проекта (обновлено 2026-09-19)

Этапы 00–05 полностью завершены и приняты; этап 06 (контракты и
MockAdapter, часть 1) выполнен 2026-09-19; этап 07 (метрики остатков) — тоже; этап 08 (правки метрик остатков, сравнение периодов, топ/анти-топ) — тоже; этап 09 (оборачиваемость со стартовым остатком, аутентификация standalone) — тоже; этап 10 (виджеты неликвидов/риска дефицита/топа, флаги, гигиена аутентификации) — тоже; этап 11 (выбор источника, справочники товаров и складов, overview) — тоже. Этапы 12
(`Bitrix24Adapter`) и 13 (`OneCAdapter`) — not planned, ждут появления
реального клиента. Прежние Known issues (см. выше) закрыты; по итогам
анализа HEAD `3ddd752` в раздел Known issues добавлены три открытых
пункта (актуальность `latestPeriodFor()`, границы `fetchDeals()`,
семантика анти-топа) — к началу этапа 12 их нужно закрыть или снять.
Проект поставлен на паузу до появления реального
клиента/доступа к Bitrix24/1С; следующий реальный шаг — начать этап 12
или 13, когда появится клиент.
