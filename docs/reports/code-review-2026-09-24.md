...
| 3.1, 3.2, 4.1 | подтверждены | исправлено 2026-09-26: серия «год назад» на обзоре без истории не рисуется (пояснение вместо нулей); `table`/`kpi-card` через `Format::num`, проценты в CSV с одним знаком; `.env.example` под Sail/PostgreSQL |
| §8 | в основном **неверен** | исторические записи не ошибки; исправлены только докблок `MockScenarioConfig` и `docs/demo.md` |

Остальное (§3.4, §4.2–4.3, §5, §7) выборочно подтверждено, латентные
мелочи — не правились. Отложенные §3.1, §3.2 и §4.1 исправлены 2026-09-26
(ветка `review-leftovers`).
