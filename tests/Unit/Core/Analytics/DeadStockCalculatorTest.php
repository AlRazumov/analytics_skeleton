...
it('uses days since the lookback start (a lower bound) and flags no_sales_in_lookback when nothing was sold', function () {
    $records = deadStock([stockMove('2026-01-01', 'p1', 'w1', T::Receipt, 10)], [], '2026-01-01', '2026-03-31');

    // Начало lookback: 2026-01-01 − 90 дней = 2025-10-03; до 2026-03-31 = 179 дней — нижняя граница.
    expect($records['p1@month:2026-03']->value)->toBe(179.0)
        ->and($records['p1@month:2026-03']->valueMeta)->toMatchArray(['no_sales_in_lookback' => true, 'stock_qty' => 10.0]);
});
...
it('takes the opening stock from fetchStock instead of summing the whole history', function () {
    ...
    // Начало lookback 2025-11-03 (28 фев − 117 дней).
    expect($records['p1@month:2026-02']->valueMeta)->toMatchArray(['stock_qty' => 50.0, 'no_sales_in_lookback' => true])
        ->and($records['p1@month:2026-02']->value)->toBe(117.0);
});
...
it('sees sales before the range start (lookback) and writes snapshots only for months of the range', function () {
    $movements = [
        stockMove('2026-01-01', 'p1', 'w1', T::Receipt, 10),
        stockMove('2026-02-15', 'p1', 'w1', T::Sale, -1),
    ];

    // Расчёт только за март: продажа 15 февраля лежит в lookback (порог 90).
    $records = deadStock($movements, [], '2026-03-01', '2026-03-31');

    expect(array_keys($records))->toBe(['p1@month:2026-03'])
        ->and($records['p1@month:2026-03']->value)->toBe(44.0)
        ->and($records['p1@month:2026-03']->valueMeta)->not->toHaveKey('no_sales_in_lookback')
        ->and($records['p1@month:2026-03']->valueMeta['stock_qty'])->toBe(9.0);
});

it('gives the same values for a month whether it is calculated alone or as part of a wider range', function () {
    $movements = [
        stockMove('2025-11-01', 'p1', 'w1', T::Receipt, 10),
        stockMove('2025-12-05', 'p1', 'w1', T::Sale, -2),
        stockMove('2026-02-20', 'p2', 'w1', T::Receipt, 5),
        stockMove('2026-03-02', 'p2', 'w1', T::Sale, -1),
    ];

    $alone = deadStock($movements, [], '2026-03-01', '2026-03-31');
    $wide = deadStock($movements, [], '2025-11-01', '2026-03-31');

    foreach (['p1@month:2026-03', 'p2@month:2026-03'] as $key) {
        expect($alone[$key]->value)->toBe($wide[$key]->value)
            ->and($alone[$key]->valueMeta['stock_qty'])->toBe($wide[$key]->valueMeta['stock_qty']);
    }
});
