 ...
 * - Товар без единой сделки за весь диапазон в расчёт не попадает —
 *   снэпшота по нему нет (список товаров классификатор не получает,
 *   только сделки). Если сделки есть, но их суммарная сумма нулевая
 *   (mean<=0), CV не определён математически — такой товар относится
 *   к Z, value=0.
 *
